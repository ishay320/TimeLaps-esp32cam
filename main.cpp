...
void configESPCamera() {
    ...
    s->set_gainceiling(s, GAINCEILING_128X);  // GAIN CEILING (0 to 6)
    
    s->set_aec2(s, 1);                        // AEC2 (0 = Disable , 1 = Enable)
    s->set_awb_gain(s, 1);                    // AWB GAIN (0 = Disable , 1 = Enable)
    s->set_agc_gain(s, 30);                    // AGC GAIN (0 to 30)
    s->set_aec_value(s, 600);                 // AEC VALUES (0 to 1200)

    s->set_special_effect(s, 0);              // SPECIAL EFFECTS (0 - No Effect, 1 - Negative, 2 - Grayscale, 3 - Red Tint, 4 - Green Tint, 5 - Blue Tint, 6 - Sepia)
    s->set_wb_mode(s, 1);                     // WB MODES (0 - Auto, 1 - Sunny, 2 - Cloudy, 3 - Office, 4 - Home)
    s->set_ae_level(s, 0);                    // AE LEVELS (-2 to 2)

    s->set_dcw(s, 1);                         // DCW (0 = Disable , 1 = Enable)
    s->set_bpc(s, 1);                         // BPC (0 = Disable , 1 = Enable)
    ...
    s->set_vflip(s, 1);                       // VERT FLIP (0 = Disable , 1 = Enable)
